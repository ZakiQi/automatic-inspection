<!-- 应用服务巡检 -->
<template>
  <inspection-table :tableTitle="'应用服务巡检'" v-show="showComponents" :count="count">
    <div style="margin-top:13px;">
      <vxe-table stripe :data="tableData" :loading="loading" :align="'center'">
        <vxe-table-column field="code" title="编号"  width="200"></vxe-table-column>
        <vxe-table-column field="typeName" title="类型" width="200"></vxe-table-column>
        <vxe-table-column field="content" title="巡检内容" :align="'left'"></vxe-table-column>
        <vxe-table-column field="codeDescribe" title="巡检名称"></vxe-table-column>
        <vxe-table-column field="status" title="状态" width="100">
          <template v-slot="{ row }">
            <span v-if="+row.status === 1" style="color: #4DAF63;">正常</span>
            <span v-else style="color: #FF4F4F;">异常</span>
          </template>
        </vxe-table-column>
      </vxe-table>
...
import { applicationListInfoCount, getApplicationListInfo } from '@/lib/api/inspection'
...
export default {
  ...
  methods: {
    initQuery (data) {
      ...
      if (data) {
        this.showComponents = data.kind === -1 || data.kind === 3
      ...
      if (this.showComponents) {
        ...
        this.initApplicationListInfo(params)
      }
    },
    
    getCount (params) {
      applicationListInfoCount(params).then(e => {
        this.count = e
      })
    },

    // 对外接口巡检表格内容初始化
    initApplicationListInfo (data) {
      this.loading = true
      const params = {
        pageNo: this.page,
      ...
      getApplicationListInfo(Object.assign(params, data)).then(e => {
        this.loading = false
